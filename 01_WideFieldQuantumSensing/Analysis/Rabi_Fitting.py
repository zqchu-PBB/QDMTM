...
class Rabi_fitting():
    ...
    def fitting(X = [], Y = [], yerr = [], save_path = [], power = []): # Variable X is a list and Y is a numpy.array.
        ...
        def func(x,magnitude,omega,t2_star,constant):
            return magnitude*np.cos(2*math.pi*omega*x)*np.exp(-(x/t2_star)**2)+constant
        ...
        init_vals = [magnitude, omega, t2_star, constant]
        print(init_vals)

        popt, pcov = curve_fit(func, x_data, y_data, p0 = numpy.array(init_vals), sigma = yerr)
        print('FITTING')

        magnitude = popt[0] # popt里面是拟合系数，读者可以自己help其用法  
        omega = popt[1]
        t2_star = popt[2]
        constant = popt[3]
        ...
        f = open(save_path+'\\Processed\\Rabi_fitting_power_'+str(power)+'dBm.txt','w')
        ...
        print(power)
        print(type(power))
        plt.savefig(save_path+'\\Processed\\Rabi_fitting_mean_power_'+str(power)+'dBm.tif',dpi=240)
        plt.close()
